...
<body>
<script src="https://cdn.statically.io/gh/akahon/zetsoft/9dab84cb/jwplayer.js"></script>
<script>jwplayer.key='dYZaZhVg8YMcuroFV8/9r+trIPGjiF5hrf264w==';</script>



<div class=big id="titulo" style="padding-top:10px;padding-left:15px;">{.breadcrumbs|{:<a href="%bread-url%">{.if|{.%bread-name%=.}|HFS |%bread-name%.} / </a>:} .}</div>
<div class=body>
<center>
...
function player(url, title, format) {
    ...
    div.innerHTML = "<div style='margin-top:30px;'>Воспроизводится файл </br> </br></div>" + title + "<div style='margin-top:70px;margin-bottom:70px'><hr style='border-color:red;'></div>";
    document.getElementById('player').style.display='block';
    document.getElementById('playertitle').style.display='block';
	var playerInstance = jwplayer('player');
	switch(format) {
		case 'video':	playerwidth = 1000;
						playerheight = 480;
						break;
...
<div id='files_outer'>
	...
	{.if not| %number% |{: <div style='font-size:200%; padding:1em;'>{.!No {.if|{.length|{.?search.}.}|results|files.}.}</div> :}|{:
        <form method='post'>
            ...
            <th>{.^sortlink|n|Названия.}{.^sortlink|e.}
            <th>{.^sortlink|d|Действия.}
            <th>{.^sortlink|s|Размер.}
            <th>{.^sortlink|t|Время.}
            <th>{.^sortlink|d|Клики.}
...
[box login]		
	<fieldset id='login'>
		<legend>Zetsoft</legend>
		<center>		
		 <img src="https://cdn.statically.io/gh/akahon/zetsoft/dda27472/zetsoft.jpg" width="50px">        
		</center>
	</fieldset>                                       
...
a.iconka { text-decoration:none; color:#47c;}
a:visited { color:#55F; }
a:hover {text-decoration:underline; }
a.iconka:hover { color:#2e2eff;}
...
.comment { font-family:tahoma, verdana, arial, helvetica, sans; font-weight:normal; font-size:7pt;  margin-top:2pt; margin-left:18pt;}
.selectedPage { font-weight:bold; font-size:larger; }
.hidden { display:none; }
                             
[file=folder=link|private]
	<tr class='{.if|{.mod|{.count|row.}|2.}|even.}'><td  width="60%">
		...
		{.if not|{.%item-ext%|~folder.}|<a href="%item-url%" > <img src="%item-icon%"> %item-name%</a>|<a href="#" onclick="player('%item-url%','%item-name%','video');" disabled> <img src="%item-icon%"> %item-name%</a>
		.}		
...
{.switch|%item-ext%|,
|mp3|
<a href="#" class="iconka" onclick="player('%item-url%','%item-name%','audio');"><i class="fa fa-play-circle fa-2x" aria-hidden="true"></i></a>
<a href="%item-url%" class="iconka"><i class="fa fa-save" style="font-size:24px"></i></a>

|mkv|
<a href="#" class="iconka" onclick="player('%item-url%','%item-name%','video');"><i class="fa fa-play-circle fa-2x" aria-hidden="true"></i></a>
<a href="%item-url%" class="iconka"><i class="fa fa-save" style="font-size:24px"></i></a>
...
|<a href="%item-url%" class="iconka"><i class="fa fa-save" style="font-size:24px;  margin-left:24%;"></i></a>
...
[+folder]
<td class='nosize'><td>%item-size%<td>%item-modified%<td>%item-dl-count%
